...
#Creating constants for use with entries and the "chars" parameter.
...
class _menu_object:
    ...
    def add_text(self,text,font,colour,textrect,alignment,awrap,events):
        ...
        #events - a tuple of events during which this text will be displayed.
...
class _menu(_menu_object):
    ...
    def update_objects(self,mx,my,lc,char):
        ...
        for obj in self.objects:
            ...
            if not 1 in obj.event_flags and update_obj:     #Ensures that the object is not disabled
                ...
                obj.event_off(4)            #Clears all button presses

                if lc:      #Changes selected/pressed flags
                    if collide:
                        if not self.clicked:
                ...
                if obj.type == 'entry' and 3 in obj.event_flags:    #Updates selected entries
    ...
    def get_pressed(self):
        #Gets the ID of all pressed buttons from the last call to update_objects
...
class Entry(_menu_object):
    def __init__(self,rect,identity,events,string,font,fcolour,textrect,align,awrap,chars,sensitive,maxlen):
        ...
        fcolour = 3 part tuple representing colour of the font
        ...
        chars = the characters that are allowed as an input
