...
import sys
sys.path.append('modules')  #Access the module folder for importing
from menu import *
...
def select_piece(location):
    ...
    if game_board[selected][0] in 'RQ':
        ...
        for i in range(x+1,8,1):
            if game_board[i+y*8] == None:   #Checks for blocking piece
                moves.append(i+y*8)
            elif game_board[i+y*8][1] != str(turn): #Checks for attacking pieces
    ...
    if game_board[selected][0] in 'BQ':
        ...
        i = selected+7
        while (i-7)%8 != 0 and (i-7)//8 != 7:   #Checks for valid moves
            if game_board[i] == None:   #Checks for blocking Piece
                moves.append(i)
            elif game_board[i][1] != str(turn): #Checks for attacking Pieces
                captures.append(i)
                break
            else:
                break
    ...
    if game_board[selected][0] == 'N':
        ...
        if x >= 2 and y <= 6:   #Checks for valid moves
            if game_board[(x-2)+(y+1)*8] == None:   #Checks for blocking Piece
                moves.append((x-2)+(y+1)*8)
            elif game_board[(x-2)+(y+1)*8][1] != str(turn): #Checks for attacking Pieces
    ...
    if game_board[selected][0] == 'K':
        ...
        if selected == 4 and wcastle%2 == 0 and \
           game_board[1] == None and game_board[2] == None and game_board[3] == None and\
           not 1 in attacked and not 2 in attacked and not 3 in attacked and not 4 in attacked:
            moves.append(2)
        if selected == 4 and -1 < wcastle < 2 and \
           game_board[5] == None and game_board[6] == None and\
           not 4 in attacked and not 5 in attacked and not 6 in attacked:
            moves.append(6)

        if selected == 60 and bcastle%2 == 0 and \
           game_board[59] == None and game_board[58] == None and game_board[57] == None and\
           not 57 in attacked and not 58 in attacked and not 59 in attacked and not 60 in attacked:
            moves.append(58)
        if selected == 60 and -1 < bcastle < 2 and \
           game_board[61] == None and game_board[62] == None and\
           not 60 in attacked and not 61 in attacked and not 62 in attacked:
            moves.append(62)

        if x >= 1 and y <= 6: #Checks for valid moves
            if game_board[(x-1)+(y+1)*8] == None:   #Checks for blocking Piece
                moves.append((x-1)+(y+1)*8)
            elif game_board[(x-1)+(y+1)*8][1] != str(turn): #Checks for attacking Pieces
...
#Notes -
#selected - a number representing where the selected piece is
#moves - List of where the selected piece can move
#captures - List of where the selected piece can capture
#game_board - List of every space and what piece resides there
#Turns Represents who has to move
...
font1 = font.Font('fonts/Bahnschrift.ttf',18)
...
#Main Loop
#Notes:
#The loops run in three main steps:
#  1. Gets inputs for each menu along with general inputs
#  2. Handles inputs for each menu and updates all running systems
#  3. Draws all of the objects to the screen for each menu

#This allows the organization of each UI into only three steps
...
while running:
    ...
    #STEP 2 - Handles inputs/updates menus

    update_menus(mx,my,lc,chars)

    if is_menu_open(wpromote_menu) or is_menu_open(bpromote_menu):  #Promotion menu
        for i in wpromote_menu.get_pressed():   #Checks selection
        ...
        for i in bpromote_menu.get_pressed():   #Checks selection
        ...
        #Handles the game board and game menu
        for c in game_menu.get_pressed():
            if c == 'new':      #Reset game button
                reset_game()
            elif c == 'quit':   #Quit game button
                running = 0
            else:
                if selected == None:    #Selects piece that was clicked on
                    select_piece(c)
                else:
                    if selected == c:   #Deselects currently selected piece
                        deselect_piece()
                    else:
    ...
    if is_menu_open(game_menu):
        #Shows which pieces are captured
        i = 0
        p = 0
    ...
    draw.rect(screen,(25,25,25),(90,90,420,420))
